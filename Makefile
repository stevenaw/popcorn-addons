...
add_license = cat $(PREFIX)/popcorn/LICENSE_HEADER | sed -e 's/@VERSION/${VERSION}/' > $(1).__hdr__ ; \
...
# source
POPCORN_SRC = ${PREFIX}/popcorn/popcorn.js
...
setup:
	...
	@@cd ./popcorn/; if [ -a Makefile ]; then make setup; else echo "no Makefile present"; fi; cd ../
...
testing: complete
	...
	@@rm -fr ${TESTING_MIRROR}/AUTHORS ${TESTING_MIRROR}/LICENSE ${TESTING_MIRROR}/LICENSE_HEADER \
	          ${TESTING_MIRROR}/Makefile ${TESTING_MIRROR}/readme.md ${TESTING_MIRROR}/.gitignore \
	          ${TESTING_MIRROR}/.gitmodules ${TESTING_MIRROR}/popcorn ${TESTING_MIRROR}/make-build-tool.py \
	          ${TESTING_MIRROR}/manifest.json ${TESTING_MIRROR}/build
	...
	@@mkdir ${TESTING_MIRROR}/popcorn; cp ${POPCORN_COMPLETE_DIST} ${TESTING_MIRROR}/popcorn/popcorn.js
...
all: clean setup popcorn plugins players parsers effects complete min
...
lint-core-tests:
	@@echo "Checking core unit tests against JSLint..."
	@@$(call run_lint, popcorn/test/popcorn.unit.js)
